...
static void as_cmd_akill(sourceinfo_t *si, int parc, char *parv[]);
static void as_cmd_akill_add(sourceinfo_t *si, int parc, char *parv[]);
static void as_cmd_akill_del(sourceinfo_t *si, int parc, char *parv[]);
static void as_cmd_akill_list(sourceinfo_t *si, int parc, char *parv[]);
...
command_t as_akill_list = { "LIST", N_("Lists all network bans"), AC_NONE, 1, as_cmd_akill_list, { .path = "" } };
command_t as_akill_sync = { "SYNC", N_("Synchronises network bans to servers"), AC_NONE, 0, as_cmd_akill_sync, { .path = "" } };

mowgli_patricia_t *as_akill_cmds;

void _modinit(module_t *m)
{
	service_named_bind_command(ADMINSERV, &as_akill);

	as_akill_cmds = mowgli_patricia_create(strcasecanon);

	/* Add sub-commands */
	command_add(&as_akill_add, as_akill_cmds);
	command_add(&as_akill_del, as_akill_cmds);
	command_add(&as_akill_list, as_akill_cmds);
	command_add(&as_akill_sync, as_akill_cmds);

	hook_add_event("user_add");
	hook_add_user_add(as_akill_newuser);
}

void _moddeinit(module_unload_intent_t intent)
{
	service_named_unbind_command(ADMINSERV, &as_akill);

	/* Delete sub-commands */
	command_delete(&as_akill_add, as_akill_cmds);
	command_delete(&as_akill_del, as_akill_cmds);
	command_delete(&as_akill_list, as_akill_cmds);
	command_delete(&as_akill_sync, as_akill_cmds);
	
	hook_del_user_add(as_akill_newuser);

	mowgli_patricia_destroy(as_akill_cmds, NULL, NULL);
}
...
static void as_cmd_akill(sourceinfo_t *si, int parc, char *parv[])
{
	...
	c = command_find(as_akill_cmds, cmd);
	if (c == NULL)
	{
